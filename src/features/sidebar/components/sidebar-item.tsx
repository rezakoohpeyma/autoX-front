...
export default function SidebarItem({ item, onSelectedId, selectedId }: SidebarItemProp): JSX.Element {
    ...
    console.log(!!item.hasChild ,!isCollapsed , isSelected)
    return (
        <div onClick={() => onSelectedId(item.id)}>
            <NavLink href={item.link} className="flex justify-between items-center h-10 rounded-lg p-3 cursor-pointer text-center">
